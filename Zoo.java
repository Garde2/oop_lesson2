import java.sql.Array;
import java.util.ArrayList;
import java.util.List;

public class Zoo {

    private Radio radio;
    private List<Animal> listAnimal = new ArrayList<>();

    public List<Animal> getListAnimal() {
        return listAnimal;
    }

    public Zoo addList(Animal animal) {
        listAnimal.add(animal);
        return this;

    }

    public void setRadio(Radio radio) {
        this.radio = radio;
    }

    public List<Sayable> getSayable() {

        List<Sayable> sayables = new ArrayList<>();
        for (var item : listAnimal) {
            sayables.add(item);
        }
        sayables.add(radio);
        return sayables;
    }

    public List<Walkable> walkables() {

        List<Walkable> walkables = new ArrayList<>();
        for (var item : listAnimal) {
            if (item instanceof Walkable)
                walkables.add((Walkable) item);
        }
        return walkables;
    }

    public List<Flyable> flyables() {

        List<Flyable> flyables = new ArrayList<>();
        for (var item : listAnimal) {
            if (item instanceof Flyable)
                flyables.add((Flyable) item);
        }
        return flyables;
    }

    public List<Swimmable> swimmables() {

        List<Swimmable> swimmables = new ArrayList<>();
        for (var item : listAnimal) {
            if (item instanceof Swimmable)
                swimmables.add((Swimmable) item);
        }
        return swimmables;
    }

    public Walkable winnerWalk() {

        int index = 0;

        for (int i = 0; i < walkables().size(); i++) {
            if (walkables().get(i).walkingSpeed() > walkables().get(index).walkingSpeed()) {
                index = i;
            }
        }
        return walkables().get(index);
    }

    public Flyable winnerFly() {

        int index = 0;

        for (int i = 0; i < flyables().size(); i++) {
            if (flyables().get(i).flyingSpeed() > flyables().get(index).flyingSpeed())
                index = i;
        }
        return flyables().get(index);
    }

    public Swimmable winnerSwim() {

        int index = 0;

        for (int i = 0; i < swimmables().size(); i++) {
            if (swimmables().get(i).swimmingSpeed() > swimmables().get(index).swimmingSpeed())
                index = i;
        }
        return swimmables().get(index);
    }

}
